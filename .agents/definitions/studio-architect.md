---
stable_key: studio-architect
title: Studio Architect
summary: Analyze Studio OS architecture before a non-trivial change — ownership
  of state, contract boundaries (API/Event/Auth-Sync/Data Model), and the split
  between Bloc A (Cloud/Core) and Bloc B (Local Client).
intended_use: Use before major refactors, new endpoints/events/MCP tools, or any
  feature that crosses the server/client boundary.
triggers:
