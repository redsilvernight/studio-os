...
export const LOCAL_CONTRACT_DIGEST = "23d8ecc064c541ae1485b948d3e203458c6af1c62f0ed7493a0a8f06cedfce4a" as const;
...
export const LOCAL_COMMANDS: readonly LocalCommandSpec[] = [
  ...
  },
  {
    ...
    "mutating": false,
    "capability": "harness.verify",
    "request": "HarnessVerifyRequest",
    "response": "HarnessVerifyResult",
    "max_request_bytes": 65536,
    "max_response_bytes": 1048576,
    "timeout_ms": 30000,
    "cancellable": false
...
export const LOCAL_SCHEMAS: Readonly<Record<string, unknown>> = {
  ...
  "HarnessPreviewRequest": {"additionalProperties":false,"description":"`renew` plans a new dedicated credential even when the tool is already\nconfigured; the previous one is revoked once the new one is in place.","properties":{"adapter_id":{"pattern":"^[a-z][a-z0-9_.-]{0,63}$","title":"Adapter Id","type":"string"},"renew":{"default":false,"title":"Renew","type":"boolean"},"workspace_id":{"format":"uuid","title":"Workspace Id","type":"string"}},"required":["workspace_id","adapter_id"],"title":"HarnessPreviewRequest","type":"object"},
  "HarnessPlan": {"$defs":{"ChangeKind":{"enum":["create","modify","delete"],"title":"ChangeKind","type":"string"},"ChangeScope":{"enum":["workspace","user"],"title":"ChangeScope","type":"string"},"HarnessChange":{"additionalProperties":false,"description":"One file-level change of a plan. Hashes and a short summary describe it;\nfile content is deliberately absent so that a plan can be shown, logged and\ndiffed without ever exposing a credential a config file might hold.\n\n`target` is relative to the workspace, or to the user's home directory when\n`scope` is `user` (the tool's own configuration, DEC-0104 §2). A user-scope\nchange describes one entry of that file and its hashes are computed on the\nentry with every credential masked.","properties":{"after_hash":{"anyOf":[{"pattern":"^[0-9a-f]{64}$","type":"string"},{"type":"null"}],"default":null,"title":"After Hash"},"before_hash":{"anyOf":[{"pattern":"^[0-9a-f]{64}$","type":"string"},{"type":"null"}],"default":null,"title":"Before Hash"},"change_id":{"pattern":"^[A-Za-z0-9_.:-]{1,200}$","title":"Change Id","type":"string"},"kind":{"$ref":"#/$defs/ChangeKind"},"scope":{"$ref":"#/$defs/ChangeScope","default":"workspace"},"summary":{"maxLength":200,"minLength":1,"title":"Summary","type":"string"},"target":{"title":"Target","type":"string"}},"required":["change_id","kind","target","summary"],"title":"HarnessChange","type":"object"}},"additionalProperties":false,"properties":{"adapter_id":{"pattern":"^[a-z][a-z0-9_.-]{0,63}$","title":"Adapter Id","type":"string"},"changes":{"default":[],"items":{"$ref":"#/$defs/HarnessChange"},"maxItems":50,"title":"Changes","type":"array"},"created_at":{"format":"date-time","title":"Created At","type":"string"},"expires_at":{"format":"date-time","title":"Expires At","type":"string"},"plan_hash":{"pattern":"^[0-9a-f]{64}$","title":"Plan Hash","type":"string"},"plan_id":{"pattern":"^[A-Za-z0-9_.:-]{1,200}$","title":"Plan Id","type":"string"},"requires_confirmation":{"const":true,"default":true,"title":"Requires Confirmation","type":"boolean"},"workspace_id":{"format":"uuid","title":"Workspace Id","type":"string"}},"required":["plan_id","adapter_id","workspace_id","plan_hash","created_at","expires_at"],"title":"HarnessPlan","type":"object"},
  ...
  "IdentityEnrollRequest": {"$defs":{"ProfileRef":{"additionalProperties":false,"description":"A local profile is one server origin on one machine. Two profiles never\nshare a secret, an outbox partition or a daemon instance.","properties":{"profile_id":{"pattern":"^[a-z][a-z0-9_.-]{0,63}$","title":"Profile Id","type":"string"},"server_origin":{"title":"Server Origin","type":"string"}},"required":["profile_id","server_origin"],"title":"ProfileRef","type":"object"}},"additionalProperties":false,"description":"Enroll this machine for the human signed in on the Desktop, without an\nadmin (A5 self-service). The daemon checks `profile` against its own server\norigin before any network call, uses `human_session` for exactly one\n`POST /api/v1/machines`, stores the returned credential in the OS keyring\nand forgets the session. Validation errors never echo their input.","properties":{"human_session":{"maxLength":4096,"minLength":16,"pattern":"^[A-Za-z0-9._~+/=-]+$","title":"Human Session","type":"string"},"machine_name":{"maxLength":200,"minLength":1,"title":"Machine Name","type":"string"},"profile":{"$ref":"#/$defs/ProfileRef"},"replace_existing":{"default":false,"title":"Replace Existing","type":"boolean"}},"required":["profile","human_session","machine_name"],"title":"IdentityEnrollRequest","type":"object"},
  "IdentityEnrollResult": {"$defs":{"ComponentId":{"enum":["desktop","bridge","daemon","workspace","knowledge","code_graph","harness","secret_store","watcher"],"title":"ComponentId","type":"string"},"HumanIdentity":{"additionalProperties":false,"description":"The person using the Desktop. Authenticated by a short-lived session\nthat lives in the renderer/dashboard; its credential is part of no local\ncontract except the single, write-only `IdentityEnrollRequest.human_session`\n(DEC-0130).","properties":{"display_name":{"maxLength":200,"minLength":1,"title":"Display Name","type":"string"},"profile":{"$ref":"#/$defs/ProfileRef"},"session_expires_at":{"anyOf":[{"format":"date-time","type":"string"},{"type":"null"}],"default":null,"title":"Session Expires At"},"user_id":{"format":"uuid","title":"User Id","type":"string"}},"required":["user_id","display_name","profile"],"title":"HumanIdentity","type":"object"},"IdentityEnrollOutcome":{"enum":["enrolled","already_enrolled"],"title":"IdentityEnrollOutcome","type":"string"},"IdentityView":{"additionalProperties":false,"description":"Everything the renderer may know about identity: who, on which machine,\nand whether the secrets behind them are usable — never the secrets.","properties":{"human":{"anyOf":[{"$ref":"#/$defs/HumanIdentity"},{"type":"null"}],"default":null},"machine":{"anyOf":[{"$ref":"#/$defs/MachineIdentity"},{"type":"null"}],"default":null},"profile":{"$ref":"#/$defs/ProfileRef"},"secrets":{"default":[],"items":{"$ref":"#/$defs/SecretReferenceStatus"},"title":"Secrets","type":"array"}},"required":["profile"],"title":"IdentityView","type":"object"},"LocalError":{"additionalProperties":false,"description":"The one structured error shape of the local stack. Never carries a\nsecret, a stack trace or an absolute local path: `message` and `details`\nare validated, bounded and safe to display or log.","properties":{"code":{"$ref":"#/$defs/LocalErrorCode"},"component":{"$ref":"#/$defs/ComponentId"},"correlation_id":{"anyOf":[{"pattern":"^[A-Za-z0-9_.:-]{1,64}$","type":"string"},{"type":"null"}],"default":null,"title":"Correlation Id"},"details":{"additionalProperties":{"anyOf":[{"type":"string"},{"type":"integer"},{"type":"number"},{"type":"boolean"},{"type":"null"}]},"default":{},"title":"Details","type":"object"},"message":{"maxLength":500,"minLength":1,"title":"Message","type":"string"},"retryable":{"title":"Retryable","type":"boolean"}},"required":["code","message","component","retryable"],"title":"LocalError","type":"object"},"LocalErrorCode":{"description":"Closed, deliberately small taxonomy. Codes describe what the caller can\nbranch on; the human message and bounded details carry the rest.","enum":["invalid_request","unknown_command","payload_too_large","protocol_incompatible","capability_missing","not_supported","cancelled","timeout","internal_error","daemon_unavailable","daemon_already_running","daemon_crashed","identity_mismatch","wrong_profile","secret_absent","secret_inaccessible","secret_revoked","keyring_unavailable","workspace_config_missing","workspace_config_invalid","workspace_moved","workspace_inaccessible","project_unavailable","feature_disabled","provider_not_installed","provider_incompatible","permission_denied","index_absent","index_corrupt","language_unsupported","plan_expired"],"title":"LocalErrorCode","type":"string"},"MachineIdentity":{"additionalProperties":false,"description":"The registered machine the daemon acts as. Its credential is held by the\ndaemon through a SecretReference and never crosses the bridge.","properties":{"machine_id":{"format":"uuid","title":"Machine Id","type":"string"},"machine_name":{"maxLength":200,"minLength":1,"title":"Machine Name","type":"string"},"profile":{"$ref":"#/$defs/ProfileRef"},"registered_at":{"format":"date-time","title":"Registered At","type":"string"}},"required":["machine_id","machine_name","profile","registered_at"],"title":"MachineIdentity","type":"object"},"ProfileRef":{"additionalProperties":false,"description":"A local profile is one server origin on one machine. Two profiles never\nshare a secret, an outbox partition or a daemon instance.","properties":{"profile_id":{"pattern":"^[a-z][a-z0-9_.-]{0,63}$","title":"Profile Id","type":"string"},"server_origin":{"title":"Server Origin","type":"string"}},"required":["profile_id","server_origin"],"title":"ProfileRef","type":"object"},"SecretKind":{"enum":["machine_credential"],"title":"SecretKind","type":"string"},"SecretReference":{"additionalProperties":false,"description":"Names where a secret lives; carries no secret material. `ref_id` is an\nopaque local handle, `lookup_key` is the store-side entry name.","properties":{"kind":{"$ref":"#/$defs/SecretKind"},"lookup_key":{"pattern":"^[A-Za-z0-9_.:-]{1,200}$","title":"Lookup Key","type":"string"},"profile":{"$ref":"#/$defs/ProfileRef"},"ref_id":{"pattern":"^[A-Za-z0-9_.:-]{1,200}$","title":"Ref Id","type":"string"},"store":{"$ref":"#/$defs/SecretStore"}},"required":["ref_id","kind","store","lookup_key","profile"],"title":"SecretReference","type":"object"},"SecretReferenceStatus":{"additionalProperties":false,"properties":{"checked_at":{"format":"date-time","title":"Checked At","type":"string"},"error":{"anyOf":[{"$ref":"#/$defs/LocalError"},{"type":"null"}],"default":null},"reference":{"$ref":"#/$defs/SecretReference"},"status":{"$ref":"#/$defs/SecretStatus"}},"required":["reference","status","checked_at"],"title":"SecretReferenceStatus","type":"object"},"SecretStatus":{"enum":["present","absent","inaccessible","revoked","wrong_profile","keyring_unavailable"],"title":"SecretStatus","type":"string"},"SecretStore":{"enum":["os_keyring","process_environment"],"title":"SecretStore","type":"string"}},"additionalProperties":false,"description":"What the renderer learns: the outcome, the new machine id and the\nrefreshed identity view — never the machine credential.","properties":{"machine_id":{"anyOf":[{"format":"uuid","type":"string"},{"type":"null"}],"default":null,"title":"Machine Id"},"outcome":{"$ref":"#/$defs/IdentityEnrollOutcome"},"view":{"$ref":"#/$defs/IdentityView"}},"required":["outcome","view"],"title":"IdentityEnrollResult","type":"object"},
...
export type BridgeCommand = "runtime.handshake" | "daemon.status" | "daemon.attach" | "daemon.start" | "daemon.stop" | "daemon.restart" | "daemon.health" | "identity.get_view" | "identity.enroll" | "workspace.validate" | "workspace.get_config" | "workspace.confirm_roots" | "workspace.git_status" | "workspace.save_config" | "knowledge.status" | "knowledge.init_vault" | "knowledge.search" | "knowledge.get_document" | "knowledge.graph_page" | "knowledge.graph_expand" | "knowledge.reindex" | "code_graph.status" | "code_graph.find_symbols" | "code_graph.graph_page" | "code_graph.graph_expand" | "code_graph.reindex" | "harness.detect" | "harness.status" | "harness.preview" | "harness.apply" | "harness.rollback" | "harness.verify" | "publication.preview" | "publication.publish";
...
export type ChangeScope = "workspace" | "user";
...
export interface HarnessVerifyRequest {
  adapter_id: string;
  workspace_id: string;
}
...
export type IdentityEnrollOutcome = "enrolled" | "already_enrolled";

export interface IdentityEnrollRequest {
  human_session: string;
  machine_name: string;
  profile: ProfileRef;
...
export type VerifyState = "unconfigured" | "configured" | "token_missing" | "verified" | "failed";
