...
- Les définitions d'agents canoniques vivent dans `.agents/definitions/` :
  `studio-architect`, `contract-guardian`, `sync-debugger` et `studio-tester`.
  Les fichiers `.claude/agents/`, `.opencode/agents/` et `.codex/agents/` sont
  des projections générées (`studio adapters check` en CI) — ne jamais les
  éditer à la main.
- Les skills projet vivent dans `.agents/skills/` : `studio-context`,
  `studio-task`, `studio-decision`, `studio-handoff`, `graphify`,
  `contract-change` et `offline-sync-testing`.
- Les règles Python, contrats, base de données, MCP, offline et transferts sont
  projetées depuis `.agents/rules/` dans le bloc généré plus bas
  (marqueurs `GENERATED RULES`) — ne jamais les éditer dans ce fichier.
