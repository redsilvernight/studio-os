...
async def studio_emit_event(
    ...
    (UUID string) generated by the caller when this call might be retried
    (offline queue replay, network timeout) — replaying the same `event_id`
    returns the original stored event rather than creating a duplicate
    ...
    async def _handler(session: AsyncSession, machine: MachineModel) -> dict[str, Any]:
        ...
        parsed_event_id = uuid4()
        if event_id is not None:
            parsed = parse_uuid(event_id, "event_id")
            if isinstance(parsed, dict):
                return parsed
            parsed_event_id = parsed
