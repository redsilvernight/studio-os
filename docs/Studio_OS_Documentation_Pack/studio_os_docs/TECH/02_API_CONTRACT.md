...
- GET /decisions
- POST /decisions
- POST /decisions/{id}/accept — `proposed` -> `accepted`, role `admin`
  strictement (une acceptation de decision est une action de gouvernance
  humaine, meme frontiere de confiance que la resolution d'une revue AI
  work, DEC-0041/DEC-0078). Rejoue sans `Idempotency-Key` (action, pas une
  creation rejouable). Depuis un autre statut que `proposed` : `409
  invalid_status_transition`. Reponse `200` = `Decision` a jour ; `404`
  sur id inconnu.
- POST /decisions/{id}/supersede — `proposed` ou `accepted` -> `superseded`,
  role `admin` strictement. Idempotence N/A (une decision deja `superseded`
  repond `409 invalid_status_transition`, jamais un second etat). Reponse
  `200` = `Decision` a jour. `404` sur id inconnu. L'ensemble des statuts
  reste `proposed|accepted|superseded` — aucun nouveau statut (DEC-0078).
...
- GET /review-queue — vue agregee, lecture seule, de tout ce qui attend une
  ...
  `PATCH /ai-work/{id}`), `Decision` en `proposed` (actionnable via
  `POST /decisions/{id}/accept` ou `/supersede`, `admin` strictement,
  DEC-0078), et evenements
