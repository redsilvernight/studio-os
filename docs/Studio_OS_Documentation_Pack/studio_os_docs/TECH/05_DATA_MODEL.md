...
`content_md5` (DEC-0014, base64 RFC 1864) : md5 presigne dans le PUT du
chemin petit fichier — seul champ reellement verifie serveur (MinIO/S3
rejette le PUT en cas de mismatch, `head_object` re-verifie a la
completion). `sha256` reste une valeur declarative du client, non verifiee
(en particulier pour le multipart, ou MinIO/S3 n'offre pas de checksum
d'objet complet via URL pre-signee).
