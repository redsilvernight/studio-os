...
- [x] Transitions de decision (2026-09-19, additif, DEC-0078) : `POST /decisions/{id}/accept` (`proposed` -> `accepted`) et `/supersede` (`proposed`/`accepted` -> `superseded`), `admin` strictement, compare-and-set sur `status` (409 `invalid_status_transition`, pas de `version` ni de migration — enum inchange) ; outils MCP `studio_accept_decision`/`studio_supersede_decision` sur les memes services (inventaire 36 = 31 historiques + 5 P8). `TECH/02/04/05/07` amendes dans le meme lot, ADR DEC-0078, `docs/DECISIONS.md` regenere. Preuve : `tests/api/test_decisions.py` (11 tests) + `tests/mcp/test_decisions.py` (12 tests) + `tests/mcp/test_uc2b_tools_metadata.py` (36 outils, execute sans DB -> 8 passed) ; `ruff`/`mypy` verts (2 erreurs baseline, 0 nouvelle) ; `dashboard/openapi.json` regenere (54 paths). Limite : la suite DB-backed n'a pas ete executee faute de PostgreSQL joignable (a confirmer) ; `dashboard/src/openapi-schema.ts` non regenere (Node indisponible — lancer `npm run openapi`).
