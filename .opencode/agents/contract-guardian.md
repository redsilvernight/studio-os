...
- Report findings only — do not modify project files unless the orchestrator explicitly asks for a fix. (`edit` requires user approval at the harness level, so any fix you propose will be gated anyway; default to report-only.)
