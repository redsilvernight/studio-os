...
6. Use Graphify when cross-file relationship information is useful, once there is code to graph. Before trusting it, verify that the files touched by the task are reflected in `E:\Graphify\Studio-OS\graphify-out\manifest.json`; if they are not, report the exact `scripts/graphify-studio.ps1` refresh command for the orchestrator instead of trusting a stale graph. Do not infer freshness from file mtimes alone. For any change crossing the Bloc A/Bloc B boundary or touching a contract file, run `graphify path`/`graphify explain` between the two sides to confirm actual dependencies instead of assuming them from file layout. (Your bash access is limited to the Graphify launcher and read-only git commands; if a refresh is needed you cannot run anything else, say so explicitly.)
