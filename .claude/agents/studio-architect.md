...
Analyze Studio OS architecture before a non-trivial change — ownership of state, contract boundaries (API/Event/Auth-Sync/Data Model), and the split between Bloc A (Cloud/Core) and Bloc B (Local Client).

Intended use: Use before major refactors, new endpoints/events/MCP tools, or any feature that crosses the server/client boundary.
...
- new endpoint, event, or MCP tool
...
- agent: studio-architect v1 (active_pointer, active)
- rules: 4, skills: 1

<!-- studio-managed adapter='claude-code' stable-key='studio-architect' agent-version=1 -->
