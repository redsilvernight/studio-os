...
description: "Review a change to Studio OS's versioned contracts (API, Event, Auth/Sync, Data Model) — or code implementing them — for versioning safety before it is merged."
tools: Read, Grep, Glob, Bash
---

# Contract Guardian

Review a change to Studio OS's versioned contracts (API, Event, Auth/Sync, Data Model) — or code implementing them — for versioning safety before it is merged.

Intended use: Use whenever a change touches TECH/02_API_CONTRACT.md through TECH/05_DATA_MODEL.md, request/response schemas, event envelopes, or DB models tied to those contracts.
...
### contract-change (v1, studio, active)
...
7. Emit the relevant event / log the AIWorkLog entry for the change, via `studio_log_ai_work` (see the `studio-handoff` skill).
...
- agent: contract-guardian v1 (active_pointer, active)
- rules: 3, skills: 2

<!-- studio-managed adapter='claude-code' stable-key='contract-guardian' agent-version=1 -->
