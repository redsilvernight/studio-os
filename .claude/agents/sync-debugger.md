...
Investigate offline-queue, idempotency, resource-claim, and resumable-transfer bugs in Studio OS — SQLite outbox replay, duplicate events, stuck/expired claims, failed multipart resume.

Intended use: Use when the root cause of a sync/offline/transfer bug is unclear.
...
- agent: sync-debugger v1 (active_pointer, active)
- rules: 3, skills: 2

<!-- studio-managed adapter='claude-code' stable-key='sync-debugger' agent-version=1 -->
