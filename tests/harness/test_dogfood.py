...
def test_dogfood_scenario_ia_to_studios_workspace_setup(tmp_path: Path):
    ...
    assert "${STUDIO_MCP_MACHINE_TOKEN}" in config["mcpServers"]["studio-os"]["headers"]["Authorization"]
    ...
    result = service_no_token.verify(HarnessVerifyRequest(workspace_id=WORKSPACE_ID, adapter_id="claude-code"))
...
def test_dogfood_opencode_configuration(tmp_path: Path):
    ...
    preview = service.preview(HarnessPreviewRequest(workspace_id=WORKSPACE_ID, adapter_id="opencode"))
    apply_result = service.apply(HarnessApplyRequest(plan_id=preview.plan_id, plan_hash=preview.plan_hash, confirmed=True))
    assert apply_result.state.value == "configured"
    ...
    assert "{env:STUDIO_MCP_MACHINE_TOKEN}" in config["mcp"]["studio-os"]["headers"]["Authorization"]
...
def test_dogfood_studios_to_studios_temp_clone(tmp_path: Path):
    ...
    for adapter_id in ["claude-code", "opencode"]:
        preview = service.preview(HarnessPreviewRequest(workspace_id=WORKSPACE_ID, adapter_id=adapter_id))
        apply_result = service.apply(HarnessApplyRequest(plan_id=preview.plan_id, plan_hash=preview.plan_hash, confirmed=True))
        assert apply_result.state.value == "configured"
...
def test_mcp_connection_chain_documented():
    ...
    Chain: Harness Config -> MCP URL -> Authorization Header -> Studi'OS MCP Server -> Auth -> Tool Call
    
    1. Harness config (.mcp.json or opencode.json) contains:
       - URL: https://studio.example/mcp
       - Authorization: Bearer ${STUDIO_MCP_MACHINE_TOKEN} (Claude) or Bearer {env:STUDIO_MCP_MACHINE_TOKEN} (OpenCode)
