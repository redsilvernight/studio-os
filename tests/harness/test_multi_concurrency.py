...
from studio_contracts.local.harness import (
    HarnessPreviewRequest, HarnessApplyRequest, HarnessVerifyRequest, VerifyState
)
...
def test_two_harnesses_same_workspace_independent_configs(tmp_path: Path):
    ...
    claude_preview = claude_service.preview(HarnessPreviewRequest(workspace_id=WORKSPACE_ID, adapter_id="claude-code"))
    opencode_preview = opencode_service.preview(HarnessPreviewRequest(workspace_id=WORKSPACE_ID, adapter_id="opencode"))
    
    claude_apply = claude_service.apply(HarnessApplyRequest(plan_id=claude_preview.plan_id, plan_hash=claude_preview.plan_hash, confirmed=True))
    opencode_apply = opencode_service.apply(HarnessApplyRequest(plan_id=opencode_preview.plan_id, plan_hash=opencode_preview.plan_hash, confirmed=True))
    ...
    claude_rollback = claude_service.rollback(HarnessRollbackRequest(rollback_id=claude_apply.rollback_id, confirmed=True))
    assert claude_rollback.state.value in ("detected", "unconfigured")
    
    opencode_result = opencode_service.verify(HarnessVerifyRequest(workspace_id=WORKSPACE_ID, adapter_id="opencode"))
...
def test_no_harnesses_works(tmp_path: Path):
    ...
    from studio_client.harness.base import DetectionState, Detection
    from studio_client.harness.base import AdapterRefusal
    
    service = _make_service(FakeAdapter(), workspace, backups_root, env)
    
    from studio_contracts.local.workspace import WorkspaceScope
    from studio_contracts.local.harness import HarnessDetectResult
...
def test_concurrency_two_operations_same_adapter(tmp_path: Path):
    ...
    preview1 = service.preview(HarnessPreviewRequest(workspace_id=WORKSPACE_ID, adapter_id="claude-code"))
    apply1 = service.apply(HarnessApplyRequest(plan_id=preview1.plan_id, plan_hash=preview1.plan_hash, confirmed=True))
    assert apply1.state.value == "configured"
    
    # Second preview should return empty changes (already configured)
    preview2 = service.preview(HarnessPreviewRequest(workspace_id=WORKSPACE_ID, adapter_id="claude-code"))
...
def test_backups_are_local_only(tmp_path: Path):
    ...
    for i in range(15):
        ...
        if preview.changes:
            apply_result = service.apply(HarnessApplyRequest(plan_id=preview.plan_id, plan_hash=preview.plan_hash, confirmed=True))
            rollback = service.rollback(HarnessRollbackRequest(rollback_id=apply_result.rollback_id, confirmed=True))
